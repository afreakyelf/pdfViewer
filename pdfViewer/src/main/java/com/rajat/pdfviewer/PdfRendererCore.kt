...
internal class PdfRendererCore(
        ...
        (currentPage - dynamicPrefetchCount..currentPage + dynamicPrefetchCount)
            ...
            .forEach { pageNo ->
                CoroutineScope(Dispatchers.IO).launch {
                    ...
                    val renderResult = renderPage(pageNo, bitmap)
                    when (renderResult) {
    ...
    suspend fun renderPage(pageNo: Int, bitmap: Bitmap): PdfRenderResult =
        withContext(Dispatchers.IO) {
            if (pageNo >= getPageCount()) {
                return@withContext PdfRenderResult.Error(
                    pageNo,
                    IllegalArgumentException("Illegal pageNo. pageNo=$pageNo, pageCount=${getPageCount()}")
                )
            }
            val cachedBitmap = getBitmapFromCache(pageNo)
            if (cachedBitmap != null) {
                return@withContext PdfRenderResult.Success(pageNo, cachedBitmap)
            }
            if (!isRendererOpen) return@withContext PdfRenderResult.Error(
                pageNo,
                IllegalStateException("Renderer is not open")
            )
            val page = openPageSafely(pageNo)
            page?.use { pdfPage ->
                return@withContext try {
                    ...
                    writeBitmapToCache(pageNo, bitmap)
                    PdfRenderResult.Success(pageNo, bitmap)
                } catch (e: Exception) {
                    PdfRenderResult.Error(pageNo, e)
                }
            } ?: return@withContext PdfRenderResult.Error(pageNo, IllegalStateException("Page is null"))
        }
